...
/*
1.Datenkapselung in der Programmierung bezeichnet die Praxis, Daten oder Informationen innerhalb einer Klasse vor externem
Zugriff zu schützen. Der direkte Zugriff auf die interne Datenstruktur wird unterbunden und erfolgt stattdessen über
definierte Schnittstellen (Black-Box-Modell). Datenkapselung entspricht Zusammenfassung von Variablen und Methoden zu
einer logischen Einheit (Objekt). Jede Klasse (Body, Vector3) ist ein Beispiel von Datenkapselung.

2.Data Hiding ist eine spezifische Strategie des Verbergens interner Details eines Objekts. Es ist eine objektorientierte
Programmiertechnik. Das Data Hiding stellt sicher, dass der Datenzugriff auf Objektvariablen beschränkt ist. Data hiding
bedeutet, die internen Daten innerhalb der Klasse zu verstecken, um ihren direkten Zugriff von außerhalb der Klasse zu
verhindern. In unserer Aufgabe wäre ein Beispiel Änderung der Sichtbarkeit der Objektvariablen von public auf private
(Body,Vector 3).

3. Links von dem Punkt steht der Hinweis auf Klasse, der die aufgerufene Methode zugehört. Das kann entweder der
Name der Klasse sein(in diesem Fall spricht man vom Aufrufen einer Klassenmethode:Spacedraw.masstoColor()),
oder Aufruf einer Objektmethode für eine Instanz der ansprechende Klasse. Objektmethoden erkennt man daran,
dass sie nur für Instanzen deren Klassen aufgerufen werden können(z.B. body.radius()).
* */
...
public class Simulation {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < bodies.length; i++) {
            double mass = Math.abs(random.nextGaussian()) * OVERALL_SYSTEM_MASS / bodies.length;

            double xMassCentre = 0.2 * random.nextGaussian() * AU;
            double yMassCentre = 0.2 * random.nextGaussian() * AU;
            double zMassCentre = 0.2 * random.nextGaussian() * AU;

            double xCurrMovement = random.nextGaussian() * 5e3;
            double yCurrMovement = random.nextGaussian() * 5e3;
            double zCurrMovement = random.nextGaussian() * 5e3;

            bodies[i] = new Body(mass,
                    new Vector3(xMassCentre, yMassCentre, zMassCentre),
                    new Vector3(xCurrMovement, yCurrMovement, zCurrMovement));
        }
        double seconds = 0;
